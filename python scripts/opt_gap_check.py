...
def transfer_data(source_db_path, target_db_path):
    ...
    # Fetch data from the 'OutputBuiltCapacity' table in the source database
    source_cursor.execute(f'SELECT scenario, region, sector, tech, vintage, capacity FROM OutputBuiltCapacity WHERE tech IN {keep_techs}')
    ...
    def insert_into_target_table(table_name, data, table_type):
        ...
        insert_query = f'''
        REPLACE INTO {table_name} (region, period, tech, {table_type}_cap, units, notes, reference, data_year, data_flags, dq_est, dq_rel, dq_comp, dq_time, dq_geog, dq_tech, additional_notes)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        '''

        prepared_data = []
        for row in data:
            scenario, region, sector, tech, vintage, capacity = row
            ...
            prepared_data.append((
                region,  # region
                vintage,  # period (for example, current year; adjust as needed)
                tech,    # tech
                capacity,  # max_cap or min_cap (using capacity as max_cap/min_cap)
                'units',  # units (default value, adjust as needed)
                f'from {scenario}',  # notes (default value, adjust as needed)
    ...
    # Insert data into both 'MinNewCapacity' and 'MaxNewCapacity' tables
    insert_into_target_table('MinNewCapacity', data, 'min')
    # insert_into_target_table('MaxNewCapacity', data, 'max')
