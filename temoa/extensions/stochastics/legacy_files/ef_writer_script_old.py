...
def GenerateScenarioTreeForEF(options, scenario_instance_factory, include_scenarios=None):
    try:
        ...
        instance_dictionary = scenario_instance_factory.construct_instances_for_scenario_tree(
            scenario_tree,
        ...
        if options.verbose or options.output_times:
            print('Time to construct scenario instances=%.2f seconds' % (time.time() - start_time))
        ...
        scenario_tree.linkInInstances(
            instance_dictionary, objective_sense=options.objective_sense, create_variable_ids=True
        )

        if options.output_times:
            print(
                'Time link scenario tree with instances=%.2f seconds' % (time.time() - start_time)
            )
...
def CreateExtensiveFormInstance(options, scenario_tree):
    ...
    binding_instance = create_ef_instance(
        scenario_tree,
    ...
    if options.verbose or options.output_times:
        print('Time to construct extensive form instance=%.2f seconds' % (time.time() - start_time))
    ...
    def write(self):
        ...
        if suf not in ['.nl', '.lp']:
            if self._solver.problem_format() == ProblemFormat.cpxlp:
                ...
                raise ValueError(
                    'Could not determine output file format. '
        ...
        symbol_map = write_ef(
            self._binding_instance, output_filename, self._options.symbolic_solver_labels
        )

        print('Extensive form written to file=' + output_filename)
        if self._options.verbose or self._options.output_times:
            print('Time to write output file=%.2f seconds' % (time.time() - start_time))
    ...
    def solve(self):
        ...
        if isinstance(self._solver, PersistentSolver):
            self._solver.compile_instance(
                self._binding_instance, symbolic_solver_labels=self._options.symbolic_solver_labels
            )
        ...
        if (not self._options.disable_warmstarts) and (self._solver.warm_start_capable()):
            action_handle = self._solver_manager.queue(
                self._binding_instance, opt=self._solver, warmstart=True, **solve_kwds
            )
        else:
            action_handle = self._solver_manager.queue(
                self._binding_instance, opt=self._solver, **solve_kwds
            )
        ...
        if self._options.verbose or self._options.output_times:
            print(
                'Time to solve and load results for the '
        ...
        print(
            '>>>THE EXPECTED SUM OF THE STAGE COST VARIABLES='
...
def EFAlgorithmBuilder(options, scenario_tree):
    ...
    if len(options.solution_writer) > 0:
        for this_extension in options.solution_writer:
            if this_extension in sys.modules:
                print(
                    'User-defined EF solution writer module='
                    + this_extension
    ...
    if isinstance(ef_solver, UnknownSolver):
        raise ValueError(
            'Failed to create solver of type='
            + options.solver_type
    ...
    if options.mipgap is not None:
        if (options.mipgap < 0.0) or (options.mipgap > 1.0):
            raise ValueError(
                'Value of the mipgap parameter for the EF '
                'solve must be on the unit interval; '
                'value specified=' + str(options.mipgap)
            )
        ef_solver.options.mipgap = float(options.mipgap)

    ef_solver_manager = SolverManagerFactory(
        options.solver_manager_type, host=options.pyro_host, port=options.pyro_port
    )
    if ef_solver_manager is None:
        raise ValueError(
            'Failed to create solver manager of type='
            + options.solver_type
            + ' for use in extensive form solve'
        )
...
def exec_runef(options):
    ...
    scenario_instance_factory = ScenarioTreeInstanceFactory(
        options.model_directory, options.instance_directory, options.verbose
    )

    if options.output_times:
        print(
            'Time to import model and scenario tree structure files=%.2f seconds'
            % (time.time() - start_time)
        )

    ef = None
    try:
        scenario_tree = GenerateScenarioTreeForEF(options, scenario_instance_factory)
        ...
        if ef is not None:
            if ef._solver_manager is not None:
                if isinstance(
                    ef._solver_manager, pyomo.solvers.plugins.smanager.phpyro.SolverManager_PHPyro
                ):
                    ef._solver_manager.release_servers(shutdown=option.shutdown_pyro_workers)
                if isinstance(
                    ef._solver_manager, pyomo.solvers.plugins.smanager.pyro.SolverManager_Pyro
                ):
            ...
            ) and (options.shutdown_pyro):
                print('Shutting down Pyro solver components')
                shutdown_pyro_components(
                    host=options.pyro_host, port=options.pyro_port, num_retries=0
                )
