    ...
    for oname, odata in block.component_map(Objective, active=True).items():
        for ndx in odata:
            ...
            for i in range(len(o_terms.variables)):
                c_rhs[
                    o_terms.variables[i].parent_component().local_name, o_terms.variables[i].index()
                ] = o_terms.linear[i]
...
def coef_FC(instance, target_tech, target_year):
    ...
    c_f = sum(
        ...
        * (
            MPL[p, t, v]
            if not GDR
            else (x ** (P_0 - p + 1) * (1 - x ** (-value(MPL[p, t, v]))) / GDR)
        )
...
def sensitivity(dat, techs):
    ...
    for t in techs:
        ...
        for v in vintages:
            ...
            c_i = (
                ...
                * (
                    LLN[t, v]
                    if not GDR
                    else (x ** (P_0 - v + 1) * (1 - x ** (-value(LLN[t, v]))) / GDR)
                )
            )
            ...
            c_s = (-1) * (
                ...
                * value(instance.SalvageRate[t, v])
                / (
                    ...
                    ** (instance.time_future.last() - instance.time_future.first() - 1)
                )
            )
        ...
        print(
            '{:>10s}\t{:>7s}\t{:>6s}\t{:>4s}\t{:>6s}\t{:>5s}\t{:>7s}\t{:>7s}\t{:>5s}\t{:>3s}\t{:>5s}'.format(
                'Tech',
                'Vintage',
                'L. RC',
                'Coef',
        ...
        for p in instance.time_optimize:
            if (p, e) in instance.EmissionLimitConstraint:
                print(
                    p,
                    e,
                    instance.dual[instance.EmissionLimitConstraint[p, e]],
                    '\t',
                    instance.slack[instance.EmissionLimitConstraint[p, e]],
                )
    ...
    for t in techs:
        ...
        for v in vintages:
            ...
            clb, cub = c.solution.sensitivity.objective(
                target_var
            )  # Coefficient lower bound, coefficient upper bound
            if cub > 1e5:
                cub = 0  # Infinity
        ...
        print(
            '{:>10s}\t{:>7s}\t{:>6s}\t{:>4s}\t{:>6s}\t{:>5s}\t{:>7s}\t{:>5s}\t{:>5s}'.format(
                'Tech',
                'Vintage',
                'L. CB',
                'Coef',
        ...
        for v in vintages:
            ...
            results.append(row)
            print(
                '{:>10s}\t{:>7g}\t{:>6.0f}\t{:>4.0f}\t{:>6.0f}\t{:>5.3f}\t{:>7.1f}\t{:>5.0f}\t{:>5.3f}'.format(
                    t,
                    v,
                    clb_s[t, v],
                    coef_CAP[t, v],
...
def explore_Cost_marginal(dat):
    ...
    optimizer = SolverFactory('cplex')
    results = optimizer.solve(instance, keepfiles=True, suffixes=['dual', 'urc', 'slack', 'lrc'])
    instance.solutions.load_from(results)
...
def plot_breakeven(years, bic, ic):
    ...
    h = plt.figure()
    ax = plt.subplot(111)
    ax.fill_between(years, 0, ic, facecolor=sen_color_map['IC'])
