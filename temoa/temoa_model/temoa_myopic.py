...
def myopic_db_generator_solver(self):
    ...
    table_list = cur_org.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'Output%'").fetchall()
    ...
    tables_group1 = ['CostFixed', 'CostVariable', 'Demand', 'EmissionLimit', 'MaxActivity', 'MaxCapacity',
                     'MinActivity', 'MinCapacity', \
                     'MaxNewCapacity', 'MinNewCapacity', 'MinNewCapacityGroup', 'MaxNewCapacityGroup',
                     'MinNewCapacityShare', \
                     'MaxNewCapacityShare', 'MinAnnualCapacityFactor', 'MaxAnnualCapacityFactor', 'MinCapacityGroup',
                     'MaxCapacityGroup', \
                     'MaxCapacityShare', 'MinCapacityShare', 'MinActivityGroup', 'MaxActivityGroup', 'MinActivityShare',
                     'MaxActivityShare', \
    ...
    for i in range(N - 1, len(time_periods) - 1):
        ...
        print('Preparing the database for the period(s): ' + str(
            [str(time_periods[j][0]) for j in range(i - (N - 1), i + 1)]))
        ...
        for table in tables_group1:
            if table in [x[0] for x in table_list]:
                cur.execute(
                    "DELETE FROM " + table + " WHERE periods > " + str(time_periods[i][0]) + " OR periods < " + str(
                        time_periods[i - (N - 1)][0]) + ";")
        ...
        for table in ['Output_CapacityByPeriodAndTech', 'Output_V_Capacity']:
            if i != (N - 1):
                ...
                for ind, row in df_linkedtechs.iterrows():
                    primary_tech = pd.read_sql_query(
                        "SELECT primary_tech FROM LinkedTechs WHERE primary_region = '" + row['regions'] + \
                    ...
                    df_primary_tech = pd.read_sql_query("SELECT * FROM " + table + " \
                                                        ...
                                                        dict_table_column[table] + " = " + str(
                        time_periods[j - 1][0]) + ";", con_org)
                    if len(df_primary_tech) == 0:
                        ...
                        if table == 'Output_V_Capacity':
                            for aux_table in ['Output_VFlow_Out', 'Output_VFlow_In']:
                                query = "DELETE FROM " + aux_table + " WHERE scenario=" + "'" + str(
                                    self.options.scenario) + "' AND \
                                tech = '" + row['tech'] + "' AND regions = '" + row['regions'] + "' AND " + \
                                        dict_table_column[table] + " = " + str(time_periods[j - 1][0])
                                cur_org.execute(query)
                ...
                for ind, row in df_primarytechs.iterrows():
                    linked_tech = pd.read_sql_query(
                        "SELECT linked_tech FROM LinkedTechs WHERE primary_region = '" + row['regions'] + \
        ...
        if i != (N - 1):
            ...
            cur_org.execute("DELETE FROM Output_CapacityByPeriodAndTech WHERE scenario=" + "'" + str(
                self.options.scenario) + "' AND t_periods>" + str(time_periods[i - N][0]))
            cur_org.execute("DELETE FROM Output_Emissions WHERE scenario=" + "'" + str(
                self.options.scenario) + "' AND t_periods>" + str(time_periods[i - N][0]))
            cur_org.execute("DELETE FROM Output_VFlow_In WHERE scenario=" + "'" + str(
                self.options.scenario) + "' AND t_periods>" + str(time_periods[i - N][0]))
            cur_org.execute("DELETE FROM Output_VFlow_Out WHERE scenario=" + "'" + str(
                self.options.scenario) + "' AND t_periods>" + str(time_periods[i - N][0]))
            ...
            cur_org.execute("DELETE FROM Output_V_RetiredCapacity WHERE scenario=" + "'" + str(
                self.options.scenario) + "' AND t_periods>" + str(time_periods[i - N][0]))
            cur_org.execute("DELETE FROM Output_Curtailment WHERE scenario=" + "'" + str(
                self.options.scenario) + "' AND t_periods>" + str(time_periods[i - N][0]))
            con_org.commit()
        ...
        for table in table_list:
            ...
            try:
                if table[0] == 'LinkedTechs':
                    cur.execute(
                        "DELETE FROM LinkedTechs WHERE primary_tech NOT IN (SELECT DISTINCT(tech) FROM Efficiency)")
                    cur.execute(
                        "DELETE FROM LinkedTechs WHERE linked_tech NOT IN (SELECT DISTINCT(tech) FROM Efficiency)")
                    ...
                    if table[0] != 'ExistingCapacity':
                        for j in range(N - 1, -1, -1):
                            ...
                            query = "SELECT DISTINCT " + ",".join(names) + \
                                    ...
                                    " WHERE vintage<" + str(
                                time_periods[0][0]) + ") AND tech NOT IN (SELECT tech FROM " + \
                            ...
                            df_table.columns = ['vintage' if x == str(time_periods[i - j][0]) else x for x in
                                                df_table.columns]
        ...
        cur.execute(
            "DELETE FROM commodities WHERE flag!='e' AND comm_name NOT IN (SELECT input_comm from Efficiency UNION SELECT output_comm from Efficiency);")
