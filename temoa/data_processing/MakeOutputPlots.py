...
class OutputPlotGenerator:
    def __init__(self, path_to_db, region, scenario, super_categories=False):
        ...
        self.folder_name = (
            os.path.splitext(os.path.basename(path_to_db))[0]
            + '_'
            + region
            + '_'
            + scenario
    ...
    def extractFromDatabase(self, mode):
        ...
        if mode == 1:
            cur.execute(
                "SELECT sector, period, tech, capacity FROM OutputNetCapacity WHERE scenario == '"
                + self.scenario
                + "' AND region LIKE '"
                + self.region
                + "'"
            ...
            cur.execute(
                "SELECT sector, period, tech, SUM(flow) FROM OutputFlowOut WHERE scenario == '"
                + self.scenario
                + "' AND region LIKE '"
                + self.region
                + "' GROUP BY sector, period, tech"
            )
            self.output_vflow = cur.fetchall()
            self.output_vflow = [list(elem) for elem in self.output_vflow]
        elif mode == 3:
            cur.execute(
                "SELECT sector, period, emis_comm, SUM(emission) FROM OutputEmission WHERE scenario == '"
                + self.scenario
                + "' AND region LIKE '"
                + self.region
                + "' GROUP BY sector, period, emis_comm"
            )
...
def GeneratePlot(args):
    ...
    if error == '':
        ...
        print(
            'Done. Look for output plot images in directory:'
            + os.path.join(options.output_dir, error)
        )
